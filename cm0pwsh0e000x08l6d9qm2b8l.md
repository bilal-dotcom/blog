...
Il est aussi possible de définir le payload après avoir sélectionné l'exploit et configuré les options. On rappelle que payload est le code qui sera exécuté sur la machine cible une fois l'exploit réussi. Après avoir configuré le payload et ses options, LHOST pour l’adresse IP et LPORT pour le port, vous exécutez l’exploit.
