...
* `gospider -S [sites] -q --blacklist[ ]`
    ...
    * l’option `-S` spécifie une fichier texte contenant les URLS de sites a analyser;
        
    * l’option `-q` rend l’analyse plus concise et désactive les logs détaillés;
        
    * l’option `--blacklist` permet d’exclurer (blacklister) soit des extensions, des domaines lors de l’analyse.
